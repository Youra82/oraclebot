...
echo "     erkennt per Python-eigener UTC-Uhr das 4h-Fenster (00/04/08/12/16/20 UTC) und"
echo "     tut bei allen anderen Aufrufen nichts -- kein CRON_TZ noetig, siehe README):"
echo "     crontab -e"
echo "     */15 * * * * /usr/bin/flock -n $(pwd)/oraclebot.lock /bin/sh -c \"sleep 60; OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 cd $(pwd) && $(pwd)/.venv/bin/python3 scripts/predict_next_barrier.py >> $(pwd)/logs/cron.log 2>&1\""
echo ""
