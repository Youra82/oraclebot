...
echo "     crontab -e"
echo "     CRON_TZ=UTC   <- NICHT weglassen, NICHT durch TZ=UTC ersetzen (siehe README):"
echo "                      cron wertet '5 0 * * *' sonst in der Server-Lokalzeitzone aus,"
echo "                      nicht in UTC -- mit 'timedatectl' prüfen, ob der Server ueberhaupt UTC ist."
echo "     5 0 * * * /usr/bin/flock -n $(pwd)/oraclebot.lock /bin/sh -c \"sleep 60; OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 cd $(pwd) && $(pwd)/.venv/bin/python3 scripts/predict_next_candle.py >> $(pwd)/logs/cron.log 2>&1\""
echo ""
